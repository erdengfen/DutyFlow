...
- 开发期结构化运行文件先落在仓库内的 `data/`、`skills/` 等目录。
- 后续如采用安装式运行，可切换到独立 `workspace_root`，例如 `~/DutyFlow/workspace/`，统一承载运行数据、外部工具、skills、知识库和长期记忆；该阶段再一并处理沙箱边界。
...
- 当前 Demo 主链路仍以仓库内 `data/` 为主；未来 workspace 化属于存储形态升级，不改变主流程顺序。
