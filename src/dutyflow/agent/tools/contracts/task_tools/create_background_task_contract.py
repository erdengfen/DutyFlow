# 本文件定义 create_background_task 工具的模型可见 contract。

CREATE_BACKGROUND_TASK_TOOL_CONTRACT = {
    "type": "function",
    "function": {
        "name": "create_background_task",
        "description": "创建一条立即进入后台执行面的异步任务，写入 data/tasks 下的结构化 Markdown 任务文件。",
        "parameters": {
            "type": "object",
            "properties": {
            ...
            "required": ["title", "goal", "success_criteria"],
        },
    },
}
...
def _self_test() -> None:
    """验证工具名稳定。"""
    assert CREATE_BACKGROUND_TASK_TOOL_CONTRACT["function"]["name"] == "create_background_task"


if __name__ == "__main__":
    _self_test()
    print("dutyflow create_background_task contract self-test passed")
