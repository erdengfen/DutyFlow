...
- `exec_cli_command`
- 后续其它携带明确命令文本、脚本片段或可执行指令字符串的内部工具
...
- [ ] PermissionGate 从“静态字段审批”升级为“静态字段 + 命令级危险解析”的两段式判断
- [ ] 为 `exec_cli_command` 接入命令风险解析结果，并把结果纳入 `allow / ask / deny` 决策
- [ ] 为命令危险判定补充稳定测试样例，覆盖只读命令、危险命令、模式差异和静态兜底
