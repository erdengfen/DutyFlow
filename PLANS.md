...
状态：进行中。已完成第 1 个功能点：正式 runtime service 骨架，只建立 queue、worker 和最小状态对象，不接业务判断。Step 6 的目标不再是单独实现“权重判断模块”，而是把当前调试期能力收束为正式常驻运行的 Agent 主循环。权重判断、硬规则和决策留痕将作为正式 loop 的内部子环节实现，而不是单独脱离 runtime 编排。
