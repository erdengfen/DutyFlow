...
- 当前结论：
  ...
  - 【已实现】`BackgroundTaskWorker` 是异步执行面入口，默认执行路径已从占位 handler 替换为正式后台 subagent 执行器。
...
- 完成后飞书回推由系统层负责。
  - 【已实现】subagent 产出 `user_visible_final_text` 或等价结果摘要。
  - 【已实现】`BackgroundTaskWorker` 从任务来源会话 `source_id` 取回推目标，缺失时退回 owner 汇报会话。
  - 【已实现】`FeedbackGateway` 负责发送给用户，不把“发送飞书消息”开放成模型自由工具。
  - 【已实现】回推失败不会把已完成任务改成执行失败，而是写入 `next_action` 等待后续重试或人工检查。
- 建议闭环：
  - 飞书消息进入前台正式 loop。
  - 模型判断需要后台任务。
...
- 【待修复】后台任务即时回复仍需避免过度承诺，尤其是“审批通过后无需人工介入一定能完成”这类表述；当前后台 subagent 执行器和完成后飞书回推链路已接入，但审批恢复后的真实续跑仍需单独验收。
...
- `src/dutyflow/agent/background_task_worker.py`
  ...
  - `run_task`
  - `resume_task`
  - `BackgroundSubagentExecutor` 默认 handler 适配
...
状态：进行中。已完成主 Agent / 后台 subagent 静态 prompt 配置收束、`ModelContextView` 概念层第一版、Working Set 确定性构造、State Delta 构造、Tool Receipt 数据结构与构造器、旧 `tool_result` 的确定性 micro-compact、Evidence Store，以及 Context Budget 第一版估算。
...
- 【已完成】保存 scope 记录：`scope_type`、`scope_id`、`account_id`、`status`、`collector_names`、`discovered_from`、来源线索、批准信息、权限错误。
...
- 【已完成】每个 scope 一个 Markdown 文件，便于人工审查和修改状态。
- 【已完成】`index.md` 只做导航和摘要，不塞所有字段。
- 【已完成】第一版保留 account 维度，避免后续多账号授权时 scope 混淆。
- 【已完成】Scope Registry 不保存同步 cursor；cursor 仍归 `sync_state`。
...
1. 【已完成】补 `docs/DATA_MODEL.md`：定义 scope 记录、索引、状态、来源、类型、account 维度和路径。
2. 【已完成】新增 `src/dutyflow/feishu/scope_registry.py`，实现 Markdown store 和最小接口。
3. 【已完成】新增 `test/test_feishu_scope_registry.py`，覆盖写入、更新、状态流转、`list_enabled()`、权限失败和 account 隔离。
