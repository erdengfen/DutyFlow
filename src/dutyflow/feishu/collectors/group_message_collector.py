# 本文件负责以 owner 用户身份拉取显式范围内的飞书群消息并落盘为 ambient_context。
...
# 关键开关：飞书群消息接口每页最多请求 50 条，避免单页过大。
DEFAULT_GROUP_MESSAGE_PAGE_SIZE = 50
# 关键开关：群消息文本预览最多保留 120 字符，供索引快速查看。
GROUP_MESSAGE_PREVIEW_CHARS = 120
...
class GroupMessageCollectResult:
    """表示 group_message_collector 单轮同步结果。"""
...
@dataclass(frozen=True)
class _ParsedGroupMessage:
    """表示已转换为 ambient_context 的单条群消息。"""

    record: AmbientContextRecord
    create_time_ms: str


class GroupMessageCollector:
    """采集用户明确授权范围内的群消息历史消息。"""
    ...
    def collect(
        ...
        *,
        start_time: int | str,
        end_time: int | str,
        page_size: int = DEFAULT_GROUP_MESSAGE_PAGE_SIZE,
        page_token: str = "",
        sort_type: str = _SORT_ASC,
        save_raw: bool = False,
    ) -> GroupMessageCollectResult:
        """按显式时间窗口拉取一个 group chat_id 的历史消息。"""
    ...
    def collect_enabled_scopes(
        ...
        *,
        ...
        end_time: int | str,
        save_raw: bool = False,
    ) -> tuple[GroupMessageCollectResult, ...]:
        """从 scope registry 读取 enabled group_chat 并逐一采集。"""
        registry = FeishuScopeRegistry(self.project_root)
        account_id = scope_account_id_from_config(config)
        scopes = registry.list_enabled(COLLECTOR_NAME, account_id=account_id)
        results: list[GroupMessageCollectResult] = []
        for scope in scopes:
            if scope.scope_type != GROUP_CHAT_SCOPE:
                continue
            result = self.collect(
                scope.scope_id,
            ...
            if result.ok:
                ...
                registry.mark_permission_denied(
                    scope.account_id,
                    scope.scope_type,
                    scope.scope_id,
...
class _CollectContext:
    """表示 group_message_collector 单轮请求上下文。"""
...
def _parse_message(
    ...
    record = AmbientContextRecord(
        ...
        fetched_at=_now_iso(),
        text=text,
        text_preview=_truncate(text, GROUP_MESSAGE_PREVIEW_CHARS),
        ...
        frontmatter_extra=_group_message_frontmatter(item, msg_type),
    )
    return _ParsedGroupMessage(record, create_time)


def _group_message_frontmatter(item: Mapping[str, Any], msg_type: str) -> dict[str, str]:
    """提取 group_message 记录专属 frontmatter。"""
...
def _result(
    ...
    return GroupMessageCollectResult(
        ok=ok,
        status=status,
...
def _summary(
    ...
    sender = _mapping(item.get("sender"))
    sender_id = _as_text(sender.get("id"))
    preview = _truncate(text, GROUP_MESSAGE_PREVIEW_CHARS)
    return f"group message {message_id} from {sender_id} type {msg_type}: {preview}"
...
def _bounded_page_size(page_size: int) -> int:
    ...
    try:
        ...
        value = DEFAULT_GROUP_MESSAGE_PAGE_SIZE
    return max(1, min(value, DEFAULT_GROUP_MESSAGE_PAGE_SIZE))
...
def _trace_id(chat_id: str, end_time: str) -> str:
    ...
    return "gmc_" + _safe_part(chat_id) + "_" + _safe_part(end_time)
...
def _require_chat_id(chat_id: str) -> None:
    """校验 collector 必须绑定明确 group chat_id。"""
    if not chat_id:
...
def _self_test() -> None:
    """验证群消息解析可生成 group_message ambient_context 记录。"""
    item = {
        "message_id": "om_grp1",
    ...
    context = _CollectContext("oc_grp1", "1778039900", "1778040100", 50, _SORT_ASC, "")
    parsed = _parse_message(item, context, CollectorBudgetGuard(CollectorBudget(COLLECTOR_NAME)), "")
    assert parsed is not None
    assert parsed.record.record_id == "gm_om_grp1"
...
if __name__ == "__main__":
    _self_test()
    print("dutyflow feishu group message collector self-test passed")
